...
| **실시간 결과 그리드** | 첫 탐지 결과 즉시 결과 탭 자동 전환, 더블클릭·우클릭으로 파일/폴더 열기 |
| **부하 수준 선택** | 낮음(~25%) / 중간(~50%, 기본) / 높음(100%) — 스레드 수 + 우선순위 동시 조정 |
| **다양한 문서 지원** | hwp/hwpx · doc/docx · xls/xlsx · ppt/pptx · pdf · txt · csv 등 |
| **이미지 OCR** | Windows OCR API(WinRT)로 이미지 텍스트 추출 (한국어 우선), 백그라운드 처리 |
| **PII 탐지** | 주민번호·전화번호·이메일·IP/MAC·신용카드·계좌번호·주소 등 10가지 |
| **검증 알고리즘** | 주민번호 체크섬, 신용카드 Luhn, IP 유효성으로 오탐 최소화 |
| **멀티스레드** | CPU 코어 수 기반 병렬 스캔, 부하 수준에 따라 자동 조정 |
| **백그라운드 처리** | OOXML 임시 압축 해제 등 모든 내부 작업이 cmd 창 없이 백그라운드 실행 |
...
1. 스캔 경로 입력 → 부하 수준 선택 (기본: 중간 ~50%) → `▶ 스캔 시작`
2. 첫 탐지 결과 발생 시 **스캔 결과 탭 자동 전환** — 스캔 중에도 실시간 확인 가능
3. **스캔 결과 탭**: 탐지된 개인정보 실시간 확인
   - 컬럼 헤더 클릭 → 3단계 정렬 (▲ 오름차 → ▼ 내림차 → 없음)
   - 탐지 유형 필터 드롭다운으로 특정 유형만 표시
   - 키워드 검색: `계약서 +2024 -임시 .xlsx` (OR·AND·NOT 조합)
   - 행 **더블클릭** → 파일 열기
   - 행 **우클릭** → 파일 열기 / 폴더 열기
