...
- CLI (`PiiScanner.exe`) + Win32 GUI (`PiiScannerUI.exe`) 두 가지 실행 방식
- Excel + HTML 리포트 자동 생성 (결과 파일 클릭 시 해당 문서 바로 열림)
- Windows OCR API로 이미지 텍스트 추출 (Windows 10 이상)
- Windows IFilter로 Office/PDF 텍스트 추출
- UTF-8 / EUC-KR 한글 인코딩 자동 감지 (바이트 시퀀스 검증 방식)
...
### GUI 버전 빌드 (PiiScannerUI.exe)

```bat
build_ui.bat
```

- 동일한 스캔 엔진 사용, Win32 GUI 추가
- 진행률 표시줄, 실시간 로그, HTML/Excel 결과 바로 열기 버튼 포함
- 결과물: `build\PiiScannerUI.exe`

> **참고**: 프로젝트 루트의 `vcpkg/` 폴더는 내부에 `.git`이 포함된 embedded git repository입니다.
> 이 폴더는 빌드에 사용되지 않으며 (build.bat / build_ui.bat 모두 vcpkg 불필요),
> `.gitignore`에 추가하거나 `git rm --cached vcpkg`로 추적에서 제외하는 것을 권장합니다.
...
- 스캔 경로/출력 경로 폴더 탐색 버튼
- 스레드 수, 최대 파일 크기, 이미지 건너뜀 옵션 설정
- 스캔 중 실시간 진행률 표시줄 및 로그
- 완료 후 HTML / Excel 결과 파일 바로 열기 버튼
...
| 파일 목록 | 파일별 경로(클릭 시 열림), 확장자, 탐지 건수, 추출 방법, 오류 |
| 상세 결과 | 탐지 건별 유형, 마스킹된 원문, 줄번호, 신뢰도, 맥락 (파일 경로 클릭 가능) |
...
- **파일 경로 클릭 시 해당 문서가 바로 열림** (`file:///` URL 방식)
...
텍스트 추출 방식:
- **일반 텍스트**: 직접 읽기 (UTF-8 / UTF-16 / EUC-KR 자동 감지 — UTF-8 멀티바이트 시퀀스 검증 방식, BOM 없는 한글 파일도 정확히 구분)
